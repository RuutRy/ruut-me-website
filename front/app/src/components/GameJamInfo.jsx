...
const formUrl =
  'https://ruut-backend-function-app.azurewebsites.net/api/gamejamSignup';
const signupsUrl =
  'https://ruut-backend-function-app.azurewebsites.net/api/gamejam';

const signupMax = 1000;
...
class GameJamInfo extends Component {
  constructor(props) {
    ...
    this.targetRegistration = new Date(2026, 0, 9, 16);
    this.targetStart = new Date(2026, 0, 31, 12); // start at 31st Jan 12:00
    this.targetEnd = new Date(2026, 1, 2, 0); // end at 2nd Feb midnight
    ...
    return (
      <>
        <section className="nes-container with-title">
          <h2 className="title">Finnish Game Jam</h2>
          <div>
            <p>{over ? 'Ensi vuonna uudestaan!' : dateString}</p>
          </div>
        </section>
        ...
        {!over && signups.length > 0 && showForm && (
          <section className="nes-container with-title">
            <h2 className="title">
              Ilmoittautumiset ({signups.length}/{signupMax})
            </h2>
            {signupEls}
          </section>
        )}
